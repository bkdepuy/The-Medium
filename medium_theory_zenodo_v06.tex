...
\fancypagestyle{body}{%
\fancyhf{}%
\fancyhead[C]{\small Version 0.6 $\cdot$ DOI: [pending] $\cdot$ CC BY 4.0}%
\fancyfoot[C]{\thepage}%
...
\label{sec:org163c171}
\subsection{The Core Problem}
\label{sec:orgcd501c5}
...
The Medium distinguishes four categories of emergent property by level (developed in Part III). \emph{Substrate emergent:} \(\iota\) characters, co-manifestation, the pre-metric coupling \(\xi\), and the substrate tendency toward ground state --- the Level 1 analog of the second law, requiring no thermodynamic vocabulary. \emph{Relational-web emergent:} mass as configuration scale \(N\), charge as net distinction character, spin as relational closure, sub-threshold combinations (quantum vacuum precursors), the configuration count \(\Omega\) underlying thermodynamic entropy, and the combination rule structure whose projection the observer reads as SU(3) \(\times\) SU(2) \(\times\) U(1). \emph{Threshold emergent:} \(G\), \(\hat{H}\), energy, time, conservation laws, \(k_B\), and the Standard Model coupling constants --- co-crystallizing at the vacuum floor \(\rho_\Lambda\), not substrate-level primitives. \emph{View-layer emergent:} spacetime, dimensionality, the Standard Model particle catalog, thermodynamic laws, and the full complexity of observable physics. The GR/QM conflict is a category error: three layers, substrate granularity, relational-web averaging, view-layer smoothness. Quantizing gravity fails because gravity is already the averaged-out result. Dark energy is a projection artifact of misread redshift. Color charge is a labeling convention, not a substrate property. Unassigned: neutrino oscillation and dark matter.
...
\emph{Changes from v0.5:}
\begin{itemize}
\item Added Derivation III: \(G\) from the vacuum floor and Hubble rate (\(G = H_0^2/4\pi\rho_\Lambda\)); 94--97$\backslash$% agreement; equivalent prediction \(\Omega_\Lambda = 2/3\)
\item Added Derivation IV: \(\hat{H}\) from the threshold condition and co-emergence with \(G\); vacuum energy catastrophe dissolved as domain error; Hubble tension identified as candidate symptom of treating co-emergent constants as independent
\item Expanded Emergent Layers with systematic per-level treatment; introduced the threshold (Planck ceiling / vacuum floor) as a named regime distinct from substrate and view layer
\item Introduced threshold-emergent category: \(G\), \(\hat{H}\), energy, conservation laws, \(k_B\), and Standard Model coupling constants co-crystallize at \(\rho_\Lambda\), not at the substrate level
\item Corrected prior misclassification: \(\hat{H}\) is threshold-emergent, not substrate-emergent
\item Added "The Standard Model and Its Constants as Perceived Observations": SM is the observer's catalog of Level 2 stability extrema read at Level 3; hierarchy problem and generations problem dissolved
\item Added "Thermodynamics and Entropy as Perceived Observations": entropy spans all levels; corrected prior claim that entropy has no substrate analog; second law's direction is Level 1, its formal statement is Level 3; \(k_B\) is a projection seam constant
\end{itemize}
...
\subsection{Axioms}
\label{sec:org6adde98}

\subsubsection{Axiom I --- The Eternal Medium}
\label{sec:org01e1888}
...
\label{sec:orge204e9d}

\subsubsection{Primitive I --- The Field}
\label{sec:org90006af}
...
\item One derivation target: combination rules from \(\iota\) mode structure \(\to\) Hamiltonian
...
\emph{Status: Established in v0.5. Threshold regime (Planck ceiling / vacuum floor) added in v0.6.}

\subsection{The Observer's Position}
\label{sec:org7407a1a}
...
\emph{Status: Current. The correspondence \(\hbar = 1\,\iota_{\mathrm{proj}}\) is an empirical match, not yet a derived result. The derivation sits at the end of the dependency chain in Unresolved Questions.}

\subsection{Emergent Layers: What Each Level Permits}
\label{sec:org3933e4d}
...
\subsubsection{Level 1 --- The Substrate}
\label{sec:org18cedbb}

\emph{What exists.} \(\iota\) events: the Medium distinguishing within itself. The \(\phi\) field: the record of distinction characters produced. Two possible characters: \(+1\) and \(-1\). The adjacency constraint: identical adjacent distinction characters produce nothing new. Nothing else belongs here.

\emph{What is defined.} Occurrence: \(\iota\) events happen. Registration: \(\phi\) records character. Co-manifestation: two \(\iota\) events sharing a distinction character, with no spatial relationship between them. Pre-metric coupling: the graded coupling intensity \(\xi\) --- boundary negotiation geometry --- that is the common ancestor of kinetic and potential energy categories. The adjacency constraint is the only rule. It is not imposed on \(\iota\) from outside. It is a logical necessity: identical adjacent distinction produces no new information.

\emph{What is not defined.} Metric. Spatial distance. Time. Temporal ordering. Causality. Energy --- neither kinetic nor potential nor total. Conservation laws. Mass, charge, spin. The Hamiltonian. \(G\), \(\hbar\), \(c\). Any dimensional quantity whatsoever. These concepts are not merely unavailable at this level --- they are inapplicable. Applying them below the projection threshold is a category error, not an approximation.

\emph{What emerges toward the next level.} Distinction characters organized by co-manifestation produce the relational web. The probability landscape of accumulated stochastic \(\iota\) interactions takes shape.

\subsubsection{The Threshold --- Planck Ceiling and Vacuum Floor}
\label{sec:org8bf83b3}
...
\emph{Level 3 --- The thermodynamic laws as perceived.} The four laws of thermodynamics are Level 3 observations of substrate and relational-web structure, with placements that differ by law.

The \emph{first law} --- energy conservation --- is threshold emergent, not view-layer constructed. It co-emerges with \(\hat{H}\) at \(\rho_\Lambda\) via Noether's theorem: time-translation symmetry under the derived Hamiltonian yields energy conservation. It does not belong to Level 3 in the same sense as the others.

The \emph{zeroth law} --- transitivity of thermal equilibrium --- is Level 3. It requires temperature, which requires energy (threshold) and a statistical distribution over energy states (view layer).

The \emph{second law} --- entropy does not decrease --- is split. Its direction is Level 1: the stochastic substrate's tendency toward ground state requires no additional postulate and no dimensional formulation. Its formal statement \(dS/dt \geq 0\) is Level 3: both \(S\) in dimensional form and \(t\) require the threshold and the observer's coordinate system. The second law is the only fundamental law that breaks time-reversal symmetry. In the Medium this is not mysterious: the asymmetry is not a property of time, which is view-layer emergent and has no intrinsic direction. The asymmetry is a property of the probability landscape, which tilts toward ground state regardless of any temporal framing the observer imposes. The observer reads that tilt as the arrow of time.

The \emph{third law} --- entropy approaches zero as temperature approaches zero --- is Level 3, requiring both \(T\) and \(S\) in dimensional form.
...
\(H_0\) --- \emph{the Hubble rate.} The large-scale \(\iota\) configuration-depth gradient at the current epoch, which the observer reads as Hubble's law. In The Medium, \(H_0\) is not a recession rate but the characteristic frequency at which large-scale \(\iota\) density gradients manifest at the projection boundary. Its SI units (\(\mathrm{s^{-1}}\)) are the same regardless of interpretation; what changes is the substrate meaning. The cosmological parameters used in the numerical verification are those standardly reported for \(H_0\) --- the same values, reinterpreted as a frequency of configuration-depth gradient rather than an expansion rate.

Both are inputs to the strain field coupling. \(G\) must be constructible from them.

\subsubsection{The Dimensional Argument}
\label{sec:org313781c}
...
\label{sec:orgb6ccfb6}

\emph{Established:} The structural form of \(G\) in Medium Theory terms. The coupling constant of the gravitational strain field is the medium's vacuum-floor compliance, \(H_0^2/\rho_\Lambda\), scaled by \(1/(4\pi)\). This is not a brute constant --- it is a ratio of medium properties. An equivalent testable prediction: \(\Omega_\Lambda = 2/3\). Agreement with two independent parameter sets at 94--97$\backslash$%.
...
\subsection{Derivation IV: \(\hat{H}\) from the Threshold Condition and Co-Emergence with \(G\)}
\label{sec:org7106cd5}
...
\subsubsection{The Hamiltonian Has a Floor}
\label{sec:orga63faad}
...
\subsubsection{Pre-Energetic Character of \(\iota\) Interaction}
\label{sec:org918a209}
...
\subsubsection{The Hamiltonian Threshold Is the Vacuum Floor}
\label{sec:orgc44d910}
...
\emph{From the Hamiltonian.} \(\hat{H}\) requires stable configurations, a metric, and time. These first appear at the projection boundary when the \(\iota\)-interaction density reaches \(\rho_\Lambda\). Below that density: raw distinction, no metric, no stable configuration, no time. The Hamiltonian first becomes well-defined at \(\rho_\Lambda\).

\emph{From \(G\).} The \(G\) derivation requires \(\rho_\Lambda\) as the stiffness of the medium. Below \(\rho_\Lambda\), no substrate activity projects to the view layer; \(G\) is not a meaningful ratio because the strain field has no projection surface from which the observer reads it. \(G\) first becomes well-defined at \(\rho_\Lambda\).
...
\subsubsection{Co-Emergence}
\label{sec:org923990e}
...
\subsubsection{The Vacuum Energy Catastrophe Reframed}
\label{sec:orgff8df16}
...
\(\hat{H} = \mathrm{proj}(\cdot)\) & Hamiltonian --- threshold-emergent; first well-defined at vacuum floor \(\rho_\Lambda\); co-emergent with \(G\) & Endpoint of combination rules derivation; not substrate-level; not view-layer constructed & Current\\[0pt]
...
\(G\) & Gravitational constant --- vacuum-floor compliance of the strain field: \(G = H_0^2/(4\pi\rho_\Lambda)\) & 94--97$\backslash$% agreement; equivalent prediction \(\Omega_\Lambda=2/3\); full derivation pending formal strain field equations & Partial\\[0pt]
...
Entropy & Spans levels: \(\Omega\) (config count) at Level 2; \(k_B\) is seam constant; \(S = k_B\ln\Omega\) at Level 3; ground tendency at Level 1 & Prior claim "no substrate analog" was imprecise; dimensional form is view-layer; \(\Omega\) and tendency are not & Current\\[0pt]
...
Color charge & Labeling convention for three-fold combination stability pattern & Dissolved as substance; whether iota mode structure generates exactly SU(3) is open & Partial\\[0pt]

...
** FATAL [F1] Foundation connection --- substantially addressed, four gaps remain
:PROPERTIES:
:DOC: 1
:TYPE: OPEN MATH
:END:
...
- [X] \(M\) defined: \(M\) is the substrate itself. Not a geometric space.
- [X] \(\iota\) is a distinction within \(M\), not an event (no sequence implied).
- [X] \(\phi: M \to \{+1,-1\}\) replaced by \(c: D \times D \to \{\mathrm{same}, \mathrm{different}\}\). Character is relational, not intrinsic.
- [X] Relations between co-existing distinctions are unavoidable (self-grounding argument: follows from singularity of \(M\), same logic as distinction itself requiring no cause).
- [X] Adjacency constraint reframed as informational, not topological.
- [X] Closure condition identified as the localization mechanism: closed structures are self-contained; open chains are unstable and must close.
- [X] Degree-2 argument stated and its implication documented (see below).

Four gaps remain open:

*** Gap 1 --- Degree-2 not yet proven
...
*Testable implication:* If degree 2 is correct, \(K_3\) is the only primitive closed structure. All higher configurations (\(K_4\), \(K_5\)) must be composite. This is consistent with QCD but must be verified against the full particle spectrum.

*Action:* Derive degree-2 from the binary split rigorously, or identify the additional structure that allows higher degree.

*** Gap 2 --- Mathematical construction of N from c not specified
...
*Action:* Write an explicit non-circular construction of \(N\) from \(c\).

*** Gap 3 --- Complex N has no source
...
*Action:* Identify the mechanism that produces complex phase in \(N\).

*** Gap 4 --- Frustrated triangle ground state not worked out
...
*Action:* Work out the ground state of the frustrated triangle under the informational adjacency constraint. Determine whether frustration degeneracy corresponds to the double degeneracy of \(\lambda = 3\).

** RESOLVED [F2] "From First Principles" on Doc 2 title page
:PROPERTIES:
:DOC: 2
:TYPE: TEXT FIX
:END:

- [X] Subtitle changed to "Structure from Substrate Primitives; Scale from Observation."

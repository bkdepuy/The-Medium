...
#+BEGIN_EXPORT latex
\begin{predbox}
\textbf{Consequence of G formula: $\Omega_\Lambda = 2/3$}\\[4pt]
This follows algebraically from $G = H_0^2/4\pi\rho_\Lambda$ substituted into the Friedmann equation. It is not an independent prediction --- if the G formula is wrong, so is $\Omega_\Lambda = 2/3$. They stand or fall together. Current best value: $0.685$, discrepancy $2.7\%$.
\end{predbox}
#+END_EXPORT
...
#+BEGIN_EXPORT latex
\begin{predbox}
\textbf{Interpretation: Hubble tension}\\[4pt]
$H_0 = 68.6$ km/s/Mpc is selected for internal consistency, not derived from the substrate. The interpretation that local measurements are biased upward by $\sim$4.4 km/s/Mpc due to local overdensity is a hypothesis without a derivation behind the bias claim. Whether large-volume surveys converge toward 68--69 km/s/Mpc is a consequence of that hypothesis, not an independent prediction.
\end{predbox}
#+END_EXPORT
...
#+BEGIN_EXPORT latex
\begin{predbox}
\textbf{Predictions from cosmological evolution:}
\begin{enumerate}\setlength\itemsep{2pt}
\item $H_0 = 68.6 \pm 0.5$ km/s/Mpc --- selected for internal consistency; convergence of large-volume surveys toward this value would be supporting evidence
\item $\Lambda$ variation --- $d\Lambda/dt = -2\Lambda \cdot H_0$; no current observational constraint at this precision
\end{enumerate}
\end{predbox}

\begin{openbox}
\textbf{Observational tensions --- may rule out current evolution equations}\\[4pt]
The cosmological evolution of $G$ and $m_\text{proton}$ implied by the two-input description conflicts with current observations:
\begin{itemize}\setlength\itemsep{2pt}
\item $\dot{G}/G \approx -1.5\times10^{-10}$/yr (from $dG/dt = -2G \cdot H_0$). Current bounds: $|\dot{G}/G| < 4$--$9\times10^{-13}$/yr (MESSENGER, LLR, pulsar timing) --- roughly 200$\times$ tighter. This rate appears observationally ruled out.
\item $\dot{m}_p/m_p \approx 7\times10^{-11}$/yr (from $dm_p/dt = m_p \cdot H_0/3$). Current bounds: $|\dot{m}_p/m_p| < 10^{-14}$/yr (atomic clocks, quasar spectroscopy) --- roughly 7000$\times$ tighter. This rate also appears observationally ruled out.
\end{itemize}
Resolving these tensions requires either a physical mechanism that suppresses local variation while allowing secular variation over Hubble timescales, or a revision of the cosmological evolution equations. Until one of these is identified, these are failed predictions of the current description.
\end{openbox}
#+END_EXPORT

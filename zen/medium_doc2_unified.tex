...
This is a definition, not a derivation. What the framework has established: if the proton is the minimum closed negotiation graph, its mass must be exactly three times the fundamental quantum. The task remaining is to derive \(\varepsilon\) from the substrate scale inputs alone.

\subsection{Estimating \(\varepsilon\) from the Two Framework Inputs}
\label{sec:org4bf1a66}

...
3 & Proton & \(3\varepsilon = 938.3\) MeV & 938.3 MeV (by construction)\\[0pt]
...
\item \textbf{Quantum states} --- proton spin-up and spin-down are degenerate eigenstates with \(E = 3\varepsilon\) (structural identification; SU(2) representation theory not yet derived)
...
\item \textbf{Gravitational Hamiltonian} --- schematic ADM structure present (\(\tfrac{1}{2}\pi^2 - \tfrac{1}{2}R\)); lapse and shift functions, extrinsic curvature tensor, and full diffeomorphism constraint not yet derived
\item \textbf{LQG connection} --- negotiation graph is structurally analogous to a spin network; \(\gamma\) appears from minimum area quantum; formulas differ by \(4\pi\) (open)
\item \textbf{Diffeomorphism invariance} --- Hamiltonian constraint structure shared with ADM; whether the full constraint algebra closes is an open calculation
\end{enumerate}

\section{Document Series and Cross-References}
\label{sec:org5e1461d}

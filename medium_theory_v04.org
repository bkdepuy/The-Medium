...
The state of the Medium is described by a substrate field φ: at every location, manifestation either has occurred or it has not.
An iota is a unit occurrence with sign disposition +1 or −1. It has no spatial extent, no duration, no location, no mass, no charge, no spin. It carries no properties in isolation. It is dimensionless.

At the projection layer, an iota occurrence corresponds to a single Planck-scale event — the threshold at which quantum and gravitational effects are simultaneously relevant.

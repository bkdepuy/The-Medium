...
\(H_0\) --- /the Hubble rate./ The large-scale \iota configuration-depth gradient at the current epoch, which the observer reads as Hubble's law. In The Medium, \(H_0\) is not a recession rate but the characteristic frequency at which large-scale \iota density gradients manifest at the projection boundary.

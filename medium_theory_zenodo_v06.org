...
#+LATEX_HEADER:   \fancyhead[C]{\small Version 0.6 $\cdot$ DOI: [pending] $\cdot$ CC BY 4.0}%
...
Physics inherited a set of primitives. Object, location, motion, sequential time, the detached external observer. These were abstracted from human-scale perceptual experience and formalized into mathematics. Every subsequent framework inherited them without examination. Thermodynamics, electromagnetism, relativity, quantum mechanics, string theory, loop quantum gravity. Each built on the same foundation.

One inherited primitive runs especially deep: the continuum. General relativity is built on a smooth differentiable manifold. Spacetime is continuous, and the theory's power depends on that continuity — curvature, geodesics, and the field equations all require it. Quantum mechanics is built on discreteness. Energy comes in quanta. Measurement produces definite outcomes from superpositions. The Planck scale marks a minimum below which the continuous description breaks down.

Both frameworks are right within their domains. The conflict appears when they are brought together. Quantizing gravity means applying discrete quantum structure to something that is, in GR, constitutively continuous. The mathematics resists because the primitives are incompatible — not because the right technique hasn't been found yet.

The incompatibilities between frameworks — GR against QM, the measurement problem, the problem of time — are not obviously technical failures awaiting mathematical fixes. This document argues they are symptoms of foundational mismatch: the inherited primitives doing load-bearing work that the frameworks never account for. That argument is made in what follows. It is not assumed here.

** The Language Problem

Quantum mechanics borrowed ordinary language and bent each term. Observer no longer means a conscious mind. Measurement no longer means careful observation. Collapse may not be a physical event at all. The borrowed terms carry associations the new meanings cannot support, and that false familiarity has made the foundations harder to examine, not easier.

This document introduces replacement vocabulary at the points where the inherited terms would otherwise mislead. The substitutions are not cosmetic. They mark places where the framework makes a different ontological commitment than standard physics does.
...
The Medium distinguishes four categories of emergent property by level (developed in Part III). /Substrate emergent:/ \iota characters, co-manifestation, the pre-metric coupling \(\xi\), and the substrate tendency toward ground state --- the Level 1 analog of the second law, requiring no thermodynamic vocabulary. /Relational-web emergent:/ mass as configuration scale \(N\), charge as net distinction character, spin as relational closure, sub-threshold combinations (quantum vacuum precursors), the configuration count \(\Omega\) underlying thermodynamic entropy, and the combination rule structure whose projection the observer reads as SU(3) \(\times\) SU(2) \(\times\) U(1). /Threshold emergent:/ \(G\), \(\hat{H}\), energy, time, conservation laws, \(k_B\), and the Standard Model coupling constants --- co-crystallizing at the vacuum floor \(\rho_\Lambda\), not substrate-level primitives. /View-layer emergent:/ spacetime, dimensionality, the Standard Model particle catalog, thermodynamic laws, and the full complexity of observable physics. The GR/QM conflict is a category error: three layers, substrate granularity, relational-web averaging, view-layer smoothness. Quantizing gravity fails because gravity is already the averaged-out result. Dark energy is a projection artifact of misread redshift. Color charge is a labeling convention, not a substrate property. Unassigned: neutrino oscillation and dark matter.
...
The four major puzzles of standard cosmology --- the entropy problem, the arrow of time, cosmological redshift, and the singular origin of the Big Bang --- are addressed in Part IV. In each case the puzzle dissolves rather than being solved, because it was generated by a category error that The Medium does not commit: treating view-layer artifacts as substrate-level facts.
...
/Changes from v0.5:/
- Added Derivation III: \(G\) from the vacuum floor and Hubble rate (\(G = H_0^2/4\pi\rho_\Lambda\)); 94--97\% agreement; equivalent prediction \(\Omega_\Lambda = 2/3\)
- Added Derivation IV: \(\hat{H}\) from the threshold condition and co-emergence with \(G\); vacuum energy catastrophe dissolved as domain error; Hubble tension identified as candidate symptom of treating co-emergent constants as independent
- Expanded Emergent Layers with systematic per-level treatment; introduced the threshold (Planck ceiling / vacuum floor) as a named regime distinct from substrate and view layer
- Introduced threshold-emergent category: \(G\), \(\hat{H}\), energy, conservation laws, \(k_B\), and Standard Model coupling constants co-crystallize at \(\rho_\Lambda\), not at the substrate level
- Corrected prior misclassification: \(\hat{H}\) is threshold-emergent, not substrate-emergent
- Added "The Standard Model and Its Constants as Perceived Observations": SM is the observer's catalog of Level 2 stability extrema read at Level 3; hierarchy problem and generations problem dissolved
- Added "Thermodynamics and Entropy as Perceived Observations": entropy spans all levels; corrected prior claim that entropy has no substrate analog; second law's direction is Level 1, its formal statement is Level 3; \(k_B\) is a projection seam constant
...
This requires a note on the ground state. The ground state and the differentiated state coexist within the Medium eternally --- there is no substrate-level time in which one precedes the other. "Correcting toward ground state" describes the relational tendency of the \iota plenum, not a temporal process. The strain field is the expression of that tendency propagating through the network. It does not require time at the substrate level; time is what the view layer constructs from the propagation.
#+END_QUOTE
...
- One derivation target: combination rules from \iota mode structure \(\to\) Hamiltonian
...
/Status: Established in v0.5. Threshold regime (Planck ceiling / vacuum floor) added in v0.6./
...
** The Projection Boundary and Natural Units
...
At the substrate there are no dimensions at all --- substrate quantities are pure counts. The dimension the observer calls /action/ is not in the substrate. It is assigned at projection, when the observer wraps one projected event in a dimensional framework.
...
/Status: Current. The correspondence \(\hbar = 1\,\iota_{\mathrm{proj}}\) is an empirical match, not yet a derived result. The derivation sits at the end of the dependency chain in Unresolved Questions./

** Emergent Layers: What Each Level Permits

The Medium's three-level structure is not merely taxonomic. Each level has a precise ontology --- what exists there, what operations and relations are defined, and what is categorically absent. Confusion about which level a concept belongs to is the root cause of most of the puzzles the framework dissolves. What follows is a systematic account of each level.

*** Level 1 --- The Substrate

/What exists./ \iota events: the Medium distinguishing within itself. The \(\phi\) field: the record of distinction characters produced. Two possible characters: \(+1\) and \(-1\). The adjacency constraint: identical adjacent distinction characters produce nothing new. Nothing else belongs here.

/What is defined./ Occurrence: \iota events happen. Registration: \(\phi\) records character. Co-manifestation: two \iota events sharing a distinction character, with no spatial relationship between them. Pre-metric coupling: the graded coupling intensity \(\xi\) --- boundary negotiation geometry --- that is the common ancestor of kinetic and potential energy categories. The adjacency constraint is the only rule. It is not imposed on \iota from outside. It is a logical necessity: identical adjacent distinction produces no new information.

/What is not defined./ Metric. Spatial distance. Time. Temporal ordering. Causality. Energy --- neither kinetic nor potential nor total. Conservation laws. Mass, charge, spin. The Hamiltonian. \(G\), \(\hbar\), \(c\). Any dimensional quantity whatsoever. These concepts are not merely unavailable at this level --- they are inapplicable. Applying them below the projection threshold is a category error, not an approximation.

/What emerges toward the next level./ Distinction characters organized by co-manifestation produce the relational web. The probability landscape of accumulated stochastic \iota interactions takes shape.

*** The Threshold --- Planck Ceiling and Vacuum Floor

The threshold is not a level. It is the boundary event at which substrate operations acquire the form the observer reads as physics. It has two faces.

/The Planck ceiling./ The dimensional signature of the substrate's phase boundary. Below the Planck scale: raw \iota negotiation, no conserved quantities, no metric, no time. The Planck length, time, and mass are the smallest intervals the observer's coordinate system can meaningfully assign to substrate events. They do not mark where the observer's tools break down. They mark where a different ontological regime ends and projection begins.

/The vacuum floor./ The minimum \iota-interaction density at which stable projection first becomes possible: \(\rho_\Lambda = 5.96 \times 10^{-27}\ \mathrm{kg\,m^{-3}}\). This is the threshold at which \(G\) and \(\hat{H}\) simultaneously crystallize. Below \(\rho_\Lambda\): no stable configuration, no metric, no time, no strain field coupling, no Hamiltonian. At \(\rho_\Lambda\): all of these become well-defined together, as a single geometric event in the plenum.

The depth of the medium between Planck ceiling and vacuum floor is \(\rho_{\mathrm{Planck}}/\rho_\Lambda \approx 10^{123}\). This ratio is the depth of the medium, not a prediction error.

/What crystallizes at the threshold./ \(G\): the medium's compliance to gravitational perturbations, well-defined only above \(\rho_\Lambda\). \(\hat{H}\): the Hamiltonian, well-defined only above \(\rho_\Lambda\). Energy, time, conservation laws: co-emergent with \(\hat{H}\) at the threshold. The fundamental constants \(G\), \(\hbar\), and \(c\): not independent free parameters, but the signature of one threshold condition expressed in multiple projections. They crystallize together. They cannot be varied independently.

*** Level 2 --- The Relational Web

/What exists./ Co-manifestation relations between \iota events. Combination rules governing the \iota relational modes. Stable configurations at stability extrema. The probability landscape: accumulated stochastic \iota interactions producing regions of higher and lower likelihood. Sub-threshold combinations: \iota patterns that have not yet reached the minimum threshold for stable projection.

/What is defined./ Adjacency: the directness of a relation between \iota events. Relational closure: how encounter patterns close within the web (one traversal: integer; two traversals: half-integer). Stability extrema: configurations where the combination rules reach a minimum. The proto-properties that project as particle properties: configuration scale \(N\) (projecting as mass), net distinction character (projecting as charge), relational closure topology (projecting as spin). The strain field tendency: stable stochastic deviations from the ground-state \iota packing produce mass imperfections; the medium tends toward ground-state correction, which is the substrate precursor of gravity.

/What is not defined./ Spatial coordinates. Temporal ordering: this level is pre-dimensional, structured but without extension. Any dimensional quantity. The Hamiltonian (a threshold/projection quantity, not defined here). An observer in the QM sense --- the relational web exists prior to any embedded observer and is not constituted by observation.

/What the view layer reads from this level./ Particles: stable configurations at stability extrema. The wave function: the view layer's representation of the probability landscape. Spacetime: the projection framework the observer constructs to give relational geometry a dimensional address. The Born rule: the view layer's account of probability density in that landscape. Wave function collapse: not a physical event; the landscape resolves one reading without collapsing.

*** Level 3 --- The View Layer

/What exists./ An embedded observer: a stable \iota configuration complex enough to sustain a systematic account of what the relational web presents. A dimensional coordinate system: length, time, mass, charge. Particles as stable configurations read at the projection boundary. Fields as the view-layer description of probability density gradients in the relational web. Spacetime as the projection framework --- real and precise at this level, absent at every level below.

/What is defined./ All of physics. The Hamiltonian \(\hat{H}\), governing how configurations evolve in projected time. \(G\), \(\hbar\), \(c\), \(k\): the seam constants where the observer's dimensional framework meets the pre-dimensional relational web. Energy, conservation laws, metric, distance, duration. The Schrödinger equation \(i\hbar\,\partial\psi/\partial t = \hat{H}\psi\): the observer's account of configuration evolution in projected time --- not a substrate input but a downstream description. Spacetime curvature: the view-layer account of large-scale probability density gradients in the relational web. Quantum fields: the continuous view-layer description of the discrete substrate combination structure.

/What this level cannot do./ Access the relational web or the substrate directly: all information arrives projected. Observe sub-threshold combinations directly: they appear only as aggregate vacuum fluctuations whose quantitative bridge to the substrate is an open derivation. Make sense of the constants from within: they appear as brute inputs because the substrate that generates them is below the projection floor. Observe entanglement as anything other than mysterious: the underlying co-manifestation is a Level 1 relation with no spatial character, invisible from Level 3 except as its projection.

/The boundary the view layer cannot cross./ Standard physics operates entirely within Level 3. Every measurement, every constant, every equation is a view-layer construct. The substrate and relational web are below the observational floor. Physics built from Level 3 observations alone will always treat the constants as fundamental, always find entanglement mysterious, and always encounter the vacuum energy catastrophe --- not because those are intractable problems, but because the level from which they become tractable is not accessible to view-layer instruments.

*** Revised Placement of Properties

Prior accounts of the framework listed the following together as ``substrate emergent'': mass, charge, spin, the quantum vacuum, the Hamiltonian, spacetime, and dimensionality. That list conflates four distinct categories and requires correction.

*Substrate emergent (Level 1).* \iota characters, co-manifestation, the pre-metric coupling \(\xi\). The tendency toward ground state: the stochastic substrate's natural drift toward maximum undifferentiation, which is the ground of the second law prior to any dimensional formulation of it.

*Threshold emergent (Phase Boundary).* \(G\), \(\hat{H}\), energy, time, conservation laws --- including the first law of thermodynamics, which co-emerges with \(\hat{H}\) via Noether's theorem. The seam constants \(k_B\), the Standard Model coupling constants \(\alpha\), \(\alpha_s\), \(G_F\), \(\sin^2\theta_W\), and the Yukawa couplings: all are observer measurements of substrate coupling ratios that have no dimensions at the substrate level. They are not free parameters. They crystallize at the projection boundary as the observer's dimensional readings of Level 2 coupling structure.

*Relational-web emergent (Level 2).* Mass as \(N\), charge as net distinction character, spin as relational closure, sub-threshold combinations (vacuum precursors), stable particle configurations, the probability landscape. The microstate configuration count \(\Omega\): dimensionless, pre-metric, the substrate of all thermodynamic entropy. The structure underlying the Standard Model's gauge symmetries and generation pattern: combination rules whose projection structure the observer reads as SU(3) \(\times\) SU(2) \(\times\) U(1), and stability extrema whose pattern the observer catalogs as three fermion generations.

*View layer emergent (Level 3).* Spacetime, dimensionality, the Schrödinger equation, the Born rule, quantum fields, the fundamental forces, all dimensional quantities. The Standard Model: particle table, gauge symmetry groups, Lagrangian, Feynman diagrams, decay rates, cross-sections. Thermodynamics: temperature, entropy \(S = k_B \ln\Omega\), the zeroth, second, and third laws, the arrow of time. These are the observer's systematic account of what the lower levels present at the projection boundary. They are valid and precise descriptions of the projection.

*Unassigned.* Neutrino oscillation and dark matter have not yet been placed with confidence. Dark energy dissolves as a projection artifact of misread redshift. Color charge dissolves as a labeling convention. Gravity has a candidate direction: mass as a stable stochastic imperfection in the \iota packing; gravity as the strain field propagating through the medium toward ground-state restoration.

*** The Standard Model and Its Constants as Perceived Observations

The Standard Model is not a description of the medium. It is the observer's systematic catalog of what the relational web presents at the view layer. This distinction matters: the Standard Model is correct, complete, and extraordinarily precise within its domain. Its domain is Level 3.

The particle table lists what stability extrema of the relational web look like when read through a dimensional coordinate system by an embedded observer. The electron is not a substrate object. It is what a specific class of stable \iota configuration appears as when the observer assigns it a mass, a charge, and a spin from within the projection framework. The quark, the neutrino, the W boson are the same: Level 2 stability extrema perceived at Level 3. The substrate contains no electrons, quarks, or gauge bosons. It contains \iota events and the combination rules that produce stability extrema. The Standard Model's particle catalog is the observer's name list for those extrema, read from below.

The gauge symmetry groups SU(3) \(\times\) SU(2) \(\times\) U(1) are the observer's mathematical description of the combination rules' projection structure --- not the combination rules themselves. The symmetry that appears as SU(3) at the view layer reflects some structure in how the relational modes combine at Level 2 that projects with threefold stability character. The observer encounters that projection, recognizes a symmetry, and names it. The mathematics correctly captures the projection structure. It does not describe what is being projected.

The Standard Model's coupling constants are projection seam constants, the same in kind as \(G\) and \(\hbar\). The fine structure constant \(\alpha \approx 1/137\), the strong coupling \(\alpha_s\), the Fermi constant \(G_F\), the Weinberg angle \(\sin^2\theta_W\), the Yukawa couplings: each is the observer's measurement of a substrate coupling ratio that has no dimensions at the substrate level. They appear as brute inputs to the Standard Model because the substrate structure that determines them is below the projection floor. They are not free. They are what specific Level 2 coupling ratios happen to be, expressed in the observer's coordinate system.

The Standard Model's \(\sim\)19 free parameters appear free because they are being read from the view layer without access to the substrate structure that determines them. From the substrate, they are no more free than \(G\) is free. The combination rules and relational mode structure at Level 2 fix them completely. The observer cannot derive them from within Level 3 and so measures them empirically and calls them fundamental. That is not a failure of the Standard Model. It is the correct epistemic position of an observer who can only see the projection.

Two apparent puzzles dissolve under this framing.

/The hierarchy problem./ Gravity is approximately \(10^{36}\) times weaker than electromagnetism. This appears anomalous when both are treated as fundamental forces of the same kind. They are not the same kind. \(G\) is a macroscopic strain field coupling set by the vacuum floor \(\rho_\Lambda\) and the Hubble rate \(H_0\) --- a cosmological-scale average emerging from the large-scale \iota density structure of the plenum. The electromagnetic coupling \(\alpha\) is a direct relational web coupling between adjacent configurations at Planck scale. Comparing them is comparing a large-scale aggregate to a microscopic interaction. The hierarchy is not a fine-tuning problem requiring a solution. It is the ratio of two scales that were never in the same category.

/The generations problem./ The Standard Model catalogs three generations of fermions --- three copies of the lepton and quark families, identical in charge and spin but differing in mass --- without explaining why three exist or why the masses differ between them. The medium places the answer at Level 2: three generations means three distinct classes of stable \iota configuration exist at stability extrema with nonzero configuration scale \(N\) and the appropriate net distinction character. The catalog is correct. The explanation is in the combination rules, which are not yet derived.

*** Thermodynamics and Entropy as Perceived Observations

Thermodynamics, like the Standard Model, is a precise and correct description of what the observer perceives at the view layer. The substrate has no temperature, no heat, no entropy in the thermodynamic sense. What it has is the ground from which those concepts emerge when a Level 3 observer constructs a statistical account of aggregate \iota activity.

The placement of thermodynamic concepts follows the same pattern as the Standard Model constants, but spans all three levels in a way that is worth stating explicitly.

/Level 1 --- The substrate ground./ The substrate at rest is maximally disordered. Pure stochastic \iota events, no structure, maximum undifferentiation. This is not a thermodynamic state --- the vocabulary of entropy does not yet apply here. But it is the condition from which the direction of entropy increase follows without any additional postulate. Ordered configurations are exponentially less probable than disordered ones in a stochastic substrate. The medium tends toward its ground state not because a law compels it but because that is what statistical behavior in a maximally random substrate means at scale. The ground of the second law is here, prior to any dimensional formulation of it.

/Level 2 --- Configuration counting./ The relational web produces a configuration space: the set of valid \iota arrangements consistent with the combination rules. The count of configurations consistent with a given macrostate --- what thermodynamics calls \(\Omega\) --- is a Level 2 quantity. It is dimensionless, pre-metric, and exists prior to any observer. The probability landscape of the relational web is the microstate distribution from which all thermodynamic behavior emerges. This is where the statistical mechanics lives before the observer arrives to dimensionalize it.

/The threshold --- \(k_B\) as projection seam constant./ Boltzmann's constant \(k_B\) is a projection seam constant, the same in kind as \(G\), \(\hbar\), and \(c\). The substrate has a dimensionless configuration count \(\Omega\). Temperature is the observer's dimensional reading of the aggregate energy distribution among configurations, which requires the Hamiltonian (threshold emergent) and a unit scale (kelvin). \(k_B\) maps between them: joules per kelvin, translating the observer's energy scale into the observer's temperature scale. It is not a fact about the substrate. It is a seam artifact of the observer's choice of two separate dimensional frameworks --- one for energy, one for temperature --- to describe what is one thing at the substrate level.

The entropy \(S = k_B \ln\Omega\) straddles the threshold in the same way \(G = H_0^2 / 4\pi\rho_\Lambda\) does: the content (\(\Omega\)) is Level 2; the dimensional form (\(k_B\), joules per kelvin) is a projection seam artifact; the full expression is what the observer reads at Level 3.

/Level 3 --- The thermodynamic laws as perceived./ The four laws of thermodynamics are Level 3 observations of substrate and relational-web structure, with placements that differ by law.

The /first law/ --- energy conservation --- is threshold emergent, not view-layer constructed. It co-emerges with \(\hat{H}\) at \(\rho_\Lambda\) via Noether's theorem: time-translation symmetry under the derived Hamiltonian yields energy conservation. It does not belong to Level 3 in the same sense as the others.

The /zeroth law/ --- transitivity of thermal equilibrium --- is Level 3. It requires temperature, which requires energy (threshold) and a statistical distribution over energy states (view layer).

The /second law/ --- entropy does not decrease --- is split. Its direction is Level 1: the stochastic substrate's tendency toward ground state requires no additional postulate and no dimensional formulation. Its formal statement \(dS/dt \geq 0\) is Level 3: both \(S\) in dimensional form and \(t\) require the threshold and the observer's coordinate system. The second law is the only fundamental law that breaks time-reversal symmetry. In the Medium this is not mysterious: the asymmetry is not a property of time, which is view-layer emergent and has no intrinsic direction. The asymmetry is a property of the probability landscape, which tilts toward ground state regardless of any temporal framing the observer imposes. The observer reads that tilt as the arrow of time.

The /third law/ --- entropy approaches zero as temperature approaches zero --- is Level 3, requiring both \(T\) and \(S\) in dimensional form.

/The arrow of time./ This is the observer's projection of the substrate's entropic gradient onto the time axis the observer constructed. There is no substrate-level time to point in a direction. The observer embeds in a mature, highly differentiated region of the \iota plenum and finds that the probability landscape tilts systematically: ordered configurations become progressively less probable in one direction and more probable in the other. That tilt is what the observer calls the direction of time. The mystery of the arrow of time dissolves when time is recognized as view-layer emergent: the arrow is not in time, it is in the landscape that the observer chose to navigate using a time coordinate.
...
In The Medium, the substrate does have a direction: it tends toward maximum undifferentiation, its ground state. This is not a thermodynamic direction in the formal sense --- it requires no entropy, no temperature, no dimensional vocabulary. It is the Level 1 ground from which the second law's asymmetry follows without additional postulate. What the substrate does not have is a global thermodynamic gradient tied to an initial condition or a cosmic clock. The apparent entropy arrow that standard cosmology treats as a cosmic fact is a view-layer phenomenon: an artifact of embedded observers constructing temporal sequences and tracking accessible macrostates from within a particular slice of configuration space.

Entropy in its formal thermodynamic form --- \(S = k_B \ln\Omega\), dimensional, requiring temperature --- is view-layer emergent. But the configuration count \(\Omega\) is a Level 2 quantity, existing independently of any observer, and the substrate tendency toward ground state is its Level 1 precursor. The prior classification that entropy has no substrate analog was imprecise. What has no substrate analog is the dimensional form. The underlying structure does.
...
** Derivation III: \(G\) from the Vacuum Floor and Hubble Rate

/Note: This derivation identifies the structural form of \(G\) in Medium Theory terms and produces 94--97 percent numerical agreement with the measured value. The remaining shortfall is explained and carries a testable equivalent prediction. A fully \(G\)-independent derivation requires the formal strain field equations, which are open work./
...
*** The Two Scale-Setting Quantities

The medium at cosmological scale has two measurable characteristic quantities:

\(\rho_\Lambda\) --- /the vacuum floor./ The minimum projected \iota activity: the medium's rest-state energy density. This is the dark energy density, the term the observer associates with the cosmological constant \(\Lambda\). Below \(\rho_\Lambda\), no substrate activity projects to the view layer. It sets the stiffness of the medium --- the resistance to being perturbed away from ground state.

\(H_0\) --- /the Hubble rate./ The large-scale \iota configuration-depth gradient at the current epoch, which the observer reads as Hubble's law. In The Medium, \(H_0\) is not a recession rate but the characteristic frequency at which large-scale \iota density gradients manifest at the projection boundary. Its SI units (\(\mathrm{s^{-1}}\)) are the same regardless of interpretation; what changes is the substrate meaning. The cosmological parameters used in the numerical verification are those standardly reported for \(H_0\) --- the same values, reinterpreted as a frequency of configuration-depth gradient rather than an expansion rate.
...
This is an exact algebraic result: the formula is precisely equivalent to the prediction:

\begin{equation}
\Omega_\Lambda = \frac{2}{3} \approx 0.667
\end{equation}

Observed: \(\Omega_\Lambda \approx 0.685\) (Planck 2018). Discrepancy: 2.7\%.

*** Interpreting the Shortfall

The 2.7\% discrepancy is the matter contribution. The Friedmann equation governs the full stress-energy budget including matter and radiation:
...
/What LQG cannot do:/ Loop quantum gravity inherits \(G\). It has no mechanism for deriving it from within its own framework because \(G\) belongs to the level beneath the spin network. The Medium is pointing toward a derivation LQG cannot make.

** Derivation IV: \(\hat{H}\) from the Threshold Condition and Co-Emergence with \(G\)

/Note: This derivation identifies the domain of validity of the Hamiltonian, shows that its threshold condition is the same event as the vacuum floor established in Derivation III, and concludes that \(G\) and \(\hat{H}\) are co-emergent. It dissolves the vacuum energy catastrophe as a domain error. The formal derivation of \(\hat{H}\) from substrate combination structure remains open work./

*** The Hamiltonian Has a Floor

The Hamiltonian is not defined at the substrate level. It is a view-layer object. Its definition presupposes three conditions: a metric (to distinguish positions), time (to define rates of change), and stable configurations (to assign energies to). None of these exist below the minimum \iota-interaction threshold sufficient to produce a metrically-embedded, temporally-ordered manifestation at the projection boundary.

Below that threshold the concepts of kinetic energy, potential energy, and energy conservation are undefined --- not approximately undefined, not defined with large uncertainty, but simply inapplicable. The Hamiltonian has a floor. Below the floor it does not exist. This is not a limitation of the formalism. It is a fact about what the formalism describes.

*** Pre-Energetic Character of \(\iota\) Interaction

At the substrate level, \iota interaction is neither kinetic nor potential energy. It is a pre-metric coupling: boundary negotiation geometry with graded coupling intensity \(\xi\). This coupling is the common ancestor of both energy categories.

Kinetic energy and potential energy are the names the projection layer gives to two aspects of what is one thing at the substrate. The Lagrangian \(L = T - V\) is a view-layer construct --- useful and correct within its domain, but not a substrate truth. The T/V distinction is emergent, not primitive. The substrate has one coupling type that the projection reads as two.

*** The Hamiltonian Threshold Is the Vacuum Floor

The minimum \iota-interaction threshold at which \(\hat{H}\) becomes well-defined is the vacuum floor \(\rho_\Lambda\) established in Derivation III.

The argument has two legs, approaching from opposite sides.

/From the Hamiltonian./ \(\hat{H}\) requires stable configurations, a metric, and time. These first appear at the projection boundary when the \iota-interaction density reaches \(\rho_\Lambda\). Below that density: raw distinction, no metric, no stable configuration, no time. The Hamiltonian first becomes well-defined at \(\rho_\Lambda\).

/From \(G\)./ The \(G\) derivation requires \(\rho_\Lambda\) as the stiffness of the medium. Below \(\rho_\Lambda\), no substrate activity projects to the view layer; \(G\) is not a meaningful ratio because the strain field has no projection surface from which the observer reads it. \(G\) first becomes well-defined at \(\rho_\Lambda\).

These are the same threshold described in two vocabularies. The \(G\) derivation sees it as a density floor. The Hamiltonian sees it as an energy/time/conservation floor. Both are projections of one condition in the plenum. The Planck scale is the dimensional signature of this threshold: Planck units mark the phase boundary where the substrate's pre-metric coupling acquires the dimensional form the observer reads as physics. Below the Planck scale: raw \iota negotiation, no conserved quantities, no metric, no time. Above the Planck scale: emergent physics, all quantities mutually consistent. Planck units are not a convenient choice of scale. They are the location of the transition.

*** Co-Emergence

\(G\) and \(\hat{H}\) do not appear separately. They crystallize together at the vacuum minimum. A medium that has reached \(\rho_\Lambda\) has both --- not one before the other, not one independently of the other.

This is co-emergence in the technical sense: neither can be defined at the substrate level without the other being simultaneously defined. The conservation laws that follow from \(\hat{H}\) via Noether's theorem --- time-translation symmetry yields energy conservation, spatial-translation symmetry yields momentum conservation --- are also part of the same package. None of them precedes the others at the substrate level. They crystallize as a unit at the vacuum floor.

The broader claim: the fundamental constants \(G\), \(\hbar\), and \(c\) are not independent quantities that happen to coexist. They are the signature of one threshold condition expressed in multiple dimensions. This is why changing one changes the others: they are facets of the same geometric crystallization event in the plenum. The constants are not free parameters tuned separately. They are one condition in multiple projections.

*** The Vacuum Energy Catastrophe Reframed

Quantum field theory predicts a vacuum energy density set by modes integrated up to the Planck scale, approximately \(10^{94}\ \mathrm{g\,cm^{-3}}\). The observed vacuum energy density is approximately \(10^{-29}\ \mathrm{g\,cm^{-3}}\). The discrepancy:

\begin{equation}
\frac{\rho_{\mathrm{Planck}}}{\rho_\Lambda} \approx 10^{123}
\end{equation}

This is standardly presented as the worst prediction in physics. In The Medium it is not a prediction error. It is a domain error.

The Hamiltonian was applied below its floor. QFT integrates vacuum fluctuation modes from zero to the Planck scale as if \(\hat{H}\) is defined throughout that range. It is not. \(\hat{H}\) first becomes well-defined at \(\rho_\Lambda\). Below \(\rho_\Lambda\) there is no projection, no metric, and no modes in any physical sense. Summing over them produces a number with no physical meaning because the formalism does not apply to the regime from which the contribution is drawn.

The observed vacuum energy density \emph{is} \(\rho_\Lambda\): the minimum \iota-interaction density at which stable projection first becomes possible. It is the vacuum floor of the medium --- the lowest state from which the Hamiltonian can be read, not the ground state of a quantum field on continuous spacetime.

The ratio \(10^{123}\) is not a puzzle. It is the ratio of two scales in The Medium: the Planck ceiling (the substrate phase boundary) and the vacuum floor (the minimum projection threshold). It is the depth of the medium, expressed as a density ratio. Reframing the cosmological constant problem as the wrong problem and recognizing that the Hamiltonian has a domain threshold are the same move.

*** What This Does and Does Not Establish

/Established./ The Hamiltonian has a floor at \(\rho_\Lambda\). That floor is the same condition as the vacuum floor of Derivation III. \(G\) and \(\hat{H}\) are co-emergent at that floor --- they crystallize together. The vacuum energy catastrophe is identified as a domain error, and the ratio \(10^{123}\) is the depth of the medium between the Planck ceiling and the vacuum floor.

/Not yet established./ The formal derivation of \(\hat{H}\) from substrate combination structure. The scalar account --- \(\hat{H} = \mathrm{proj}(\Kappa)\), \(\Kappa = \sum \kappa\), \(\kappa = \sum \iota\) --- correctly identifies the endpoint and the three-level structure but uses scalar summation, which is insufficient for qualitatively distinct \iota relational modes. The combination rules must be rebuilt from the mode structure upward before \(\hat{H}\) can be derived rather than identified. That is open work.

/Prediction carried./ If the constants are co-emergent from a single threshold condition, then \(H_0\) is not a free parameter. Its value follows from the same geometric condition that fixes \(G\) and \(\hat{H}\). The medium formula, reconstructed for a preferred \(G\), gives \(H_0 \approx 68.6\) km/s/Mpc --- consistent with both the Planck 2018 CMB value and the midpoint of the Hubble tension range. The framework does not allow the Hubble constant to be adjusted independently of \(G\). The Hubble tension is therefore a candidate symptom of treating co-emergent constants as though they were independent.
...
*What the Field Measures.* The framework records distinction characters as \(\{+1,-1\}\). The original argument for binary: one act of distinction creates one boundary; one boundary has two sides. That argument assumed \iota is point-like --- making one clean cut, two sides, done.
...
\iota mode structure \(\to\) combination rules \(\to\) stable configurations \(\to\) projection boundary account \(\to\) action dimension derivation \(\to\) \(\hbar\) as derived result
...
*Color Charge --- Dissolved as a substance; SU(3) derivation open.* Color charge is never directly observed. Confinement guarantees that no colored object reaches a detector. Every observed state is color-neutral. The label was borrowed from optics for one reason: red + green + blue = white. Three things combine to produce neutrality. That is the entire justification for "color." The physics is the neutrality --- the three-fold combination rule that produces stability. The optical analogy adds nothing. It names the pattern without explaining it.
...
#+CAPTION: Complete symbol table for version 0.6
...
| \(\hat{H} = \mathrm{proj}(\cdot)\)  | Hamiltonian --- threshold-emergent; first well-defined at vacuum floor \(\rho_\Lambda\); co-emergent with \(G\) | Endpoint of combination rules derivation; not substrate-level; not view-layer constructed | Current  |
...
| \(G\)                               | Gravitational constant --- vacuum-floor compliance of the strain field: \(G = H_0^2/(4\pi\rho_\Lambda)\) | 94--97\% agreement; equivalent prediction \(\Omega_\Lambda=2/3\); full derivation pending formal strain field equations | Partial |
...
| Entropy                             | Spans levels: \(\Omega\) (config count) at Level 2; \(k_B\) is seam constant; \(S = k_B\ln\Omega\) at Level 3; ground tendency at Level 1 | Prior claim "no substrate analog" was imprecise; dimensional form is view-layer; \(\Omega\) and tendency are not | Current        |
...
| Color charge                        | Labeling convention for three-fold combination stability pattern          | Dissolved as substance; whether iota mode structure generates exactly SU(3) is open | Partial |

...
The sole primitive is \iota: a unit occurrence. \iota carries no attribute in isolation. No sign, no disposition, no property. The Medium is the endless occurrence of \iota, unconstrained. No boundary, structure, or assumed constraint exists at the substrate level. Space, time, dimensionality, and causality are not conditions imposed upon \iota --- they emerge from \iota-encounter relations. The Planck quantum of action marks the projection boundary between the substrate and observable physics, where one \iota event corresponds to \(\hbar\).

The central organizing claim of v0.5: the universe is not a sequence of events. It is a web of processes in continuous, simultaneous, mutual interaction. Sequence is what that web looks like from inside an embedded process. Attribute is what encounter produces. \iota in isolation has neither.

When two occurrences meet, a relational character is established between them. That character is not in either occurrence --- it is between them. It is the first distinguishable fact. The substrate field \(\phi\) records encounter characters. It is not a description of \iota in isolation.

From encounter characters, a three-level algebraic hierarchy is derived by successive combination. At the first level, \(\kappa = \sum \iota\) is the coupling character of a local collection --- the net encounter character of a group of \iota events. At the second level, \(\Kappa = \sum \kappa\) is the total configuration energy of a candidate stable structure. At the third level, \(\Kappa\) projects into spacetime as \(\hat{H}\), the Hamiltonian of standard physics --- not assumed but derived from substrate structure.

Valid combinations are governed by a logical adjacency constraint: two encounters of identical character produce no new relational information and therefore cannot occupy adjacent positions in a combination. This is not a physical rule imposed on \iota but a logical necessity --- identical adjacent encounter characters produce no distinction. The combinatorial space of valid configurations is sharply restricted by this constraint. A minimum threshold of combinatorial complexity must be reached before any stable configuration can form --- below this threshold, combinations approach but do not achieve coherence, naturally populating the quantum vacuum as substrate fluctuations.

Stable configurations emerge where \(\Kappa\) reaches a local extremum. These are the substrate analogs of bound states. Mass, charge, and spin are not independent intrinsic properties --- they are three projection signatures of a single underlying combinatorial structure. Mass corresponds to \(N\), the count of \iota events. Charge corresponds to the net encounter character of \(\Kappa\). Spin describes the relational closure of a configuration's encounter pattern within the web. It is not rotation.
...
\iota is none of these. It carries nothing. The distinction between encounter characters does not exist before encounter creates it. All three prior frameworks place entities first and derive relationships. The Medium places the web of simultaneous encounter first. The relationships are the primitive. The entities are downstream.

Planck bits come closest --- both are discrete, dimensionless, pre-spatial. The gap is not small. A bit carries its state intrinsically. \iota carries nothing. That is the difference between assuming distinction and deriving it.

Loop quantum gravity comes closest structurally --- both make spacetime emergent from relational structure. But LQG presupposes the relational structure. The Medium derives it from encounter.
...
The Medium possesses one property: intrinsic distinction, denoted \iota. It is not an operator, not a function, not a process. It is what the Medium is --- the capacity for occurrence. \iota carries no attribute in isolation. Attribute is produced at encounter.

/Status: Axiomatic. Character of encounter under investigation./
...
\iota is occurrence. It has no spatial extent, no duration, no location, no mass, no charge, no spin, no sign. It carries no attribute in isolation. It is dimensionless.

The substrate field \(\phi\) describes the registered character of encounters --- not properties of \iota itself. At every location where registration has occurred, encounter character has been established. \(\phi\) maps that record.
...
#+BEGIN_CENTER
A record of encounter characters. Not a description of \iota in isolation.
#+END_CENTER
...
\iota occurrences are not driven. No prior state determines the next. They simply occur.

/Status: Revised in v0.5./

*** Primitive II --- Encounter and Attribution

;; DRAFT — language needs refinement

When two occurrences meet, a relational character is established between them. That character is not in either occurrence. It is between them. It is the first distinguishable fact in the Medium.

Before encounter, there is no attribute. Not hidden. Not waiting. The question does not exist until encounter makes it exist.

\begin{equation}
\phi(x) \in \{+1, -1\} \text{ (character of encounter, no preference, no guarantee)}
\end{equation}

The substrate imposes no preference on which character is established. Without differential encounter character, accumulation of \iota occurrences produces only undifferentiated noise. No structure, no stability, no distinction, no material. Differential encounter character is the logical necessity for material existence.

This is where topos theory bears on the framework. Topos theory allows logic in which a question has no answer until the context that makes the question meaningful exists. Encounter character is not determined before encounter. The question --- which character? --- is not formed until two occurrences meet. This is not epistemic. The answer does not exist yet. That is the logical territory topos theory describes, and it is the logical territory \iota occupies before registration.
...
*Level 0 --- The Primitive.* \iota is the sole primitive. A unit occurrence. Dimensionless. No attribute in isolation. Attribute is produced at encounter, not carried by \iota itself.
...
\(\kappa\) is the net encounter character of a local cluster. It is the first derived quantity in the hierarchy.
...
*The Adjacency Constraint.* Valid combinations must satisfy: two encounters of identical character cannot occupy adjacent positions. This is not a physical rule but a logical necessity --- adjacent identical encounter characters produce no new relational information and therefore no new structure. The constraint sharply restricts the combinatorial space and is the source of the minimum complexity threshold below which stable configurations cannot form.
...
- One organizing claim: the simultaneity primitive --- the relational web of continuous mutual encounter
- One property: \iota --- occurrence without attribute
- One primitive event: encounter --- the source of relational character
- One field: \(\phi \in \{+1, -1\}\) --- a record of encounter characters
...
*Substrate emergent.* These properties arise directly from \iota-combinations, prior to any observer: mass (corresponding to \(N\), the count of \iota events in a configuration), charge (corresponding to the net encounter character of \(\Kappa\)), spin (arising from relational closure of the configuration's encounter pattern within the web), the quantum vacuum (substrate fluctuations below the stability threshold), the Hamiltonian (as \(\mathrm{proj}(\Kappa)\)), spacetime (as the projection framework constructed by embedded observers), and dimensionality (as the coordinate structure observers impose on substrate relationships).
...
At the substrate level, two locations carrying the same \(\phi\) value share a state with no spatial relationship between them. There is no distance at the substrate level. There is no /between/.
...
At the substrate level, \iota occurrences are not driven. No prior state determines whether the next occurs. When two occurrences meet, encounter either produces relational character or it does not --- with no preference and no guarantee. An observer describing this stochastic character mathematically recognizes it as a Bernoulli process --- a binary outcome with associated probability \(p(x)\) at each location. This is the observer's mathematical framework imposed on the substrate's stochastic character. The substrate does not know about Bernoulli. Occurrences simply happen. Encounters simply produce or do not.
...
*The Derivation of Each Constant.* \(c\) from the dimensionless transition bound. \(G\) from structure formation geometry. Thermodynamic entropy from the substrate's undifferentiation measure. Each is an open derivation, explicitly deferred.

*Particle Configurations.* What precisely are the stable configurations corresponding to observed particles? The formal derivation of particle properties from the \(\Kappa\) extremum condition --- beginning with the lightest candidates as threshold cases --- is the primary structural task of the next stage of development.

*Spin as Relational Closure.* Which encounter patterns produce which spin values (0, 1/2, 1, 3/2, 2)? The claim that spin is a property of relational closure --- integer spin closing in one traversal of the web, half-integer in two --- is current. The derivation of which specific encounter patterns correspond to which values is open.
...
|-------------------------------------+---------------------------------------------------------------------------+--------------------------------------------------+-------------|
| \iota                               | Intrinsic distinction --- the sole primitive, occurrence without attribute | No attribute in isolation; attribute produced at encounter | Axiomatic   |
...
| \(\kappa = \sum \iota\)             | Coupling character --- net encounter character of a local cluster         | First derived quantity                           | Current     |
...
| \(\mathrm{sgn}(\Kappa)\)            | Net encounter character of configuration energy --- projects to charge    | Charge neutrality = exact character balance      | Current     |
...
| Adjacency constraint                | Identical adjacent encounter characters forbidden --- source of minimum threshold | Logical, not physical                     | Current     |

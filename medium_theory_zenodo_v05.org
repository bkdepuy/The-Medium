...
From distinction characters, a three-level algebraic hierarchy emerges by successive combination. A scalar account derives \(\kappa = \sum \iota\) and \(\Kappa = \sum \kappa\), with \(\Kappa\) projecting into spacetime as \(\hat{H}\), the Hamiltonian of standard physics --- not assumed but derived from substrate structure. That scalar account is provisional: the multifaceted \iota model requires richer combination rules operating on the tensor product space of \iota relational modes. The Hamiltonian endpoint survives revision.
...
The Medium distinguishes three categories of emergent property. Properties emergent from the substrate arise directly from \iota-combinations: mass, charge, spin, the quantum vacuum, the Hamiltonian, spacetime, and dimensionality. Properties emergent at the view layer arise from interactions between stable configurations in spacetime: the fundamental forces, fields, and the full complexity of observable reality. This is the domain of existing physics --- The Medium makes no claims there. A third category remains unassigned: neutrino oscillation and dark matter have not yet been placed with confidence. Dark energy dissolves as a projection artifact of misread redshift. Gravity has a candidate direction: mass is a stable stochastic imperfection in the \iota packing; gravity is the strain field propagating through the medium as it attempts to correct toward ground state. Infinite reach without a carrier particle is the expected consequence; formal development is open. Color charge dissolves as a labeling convention --- the physics is the stability pattern, not the label. The GR/QM conflict dissolves across the three layers: substrate granularity, relational-web averaging, view-layer smoothness. Quantizing gravity fails because gravity is already the averaged-out result.
...
- Topos theory connection placed at the encounter level
...
Three existing anomalies fit this pattern precisely.

The proton radius puzzle: the proton's charge radius measured in muonic hydrogen gives a value approximately 4% smaller than the value measured in ordinary hydrogen. Same particle. Different measurement context --- a muon orbiting instead of an electron. Different result. Standard physics has no clean explanation within the intrinsic property framework. In the iota model this is expected: the proton's configuration projects differently depending on what it is projecting through. The measurement context is part of the relational structure.

The anomalous magnetic moment of the muon: the muon's g-factor deviates persistently from QED predictions. Standard physics interprets this as a signal of undiscovered particles. The iota model offers a different reading: the muon's relational configuration has context-dependence that the intrinsic property model absorbs into unexplained correction terms rather than recognizing as structural.

Quantum contextuality: already experimentally confirmed by Kochen-Specker theorem violations. Measurement outcomes depend on what else is being measured simultaneously. Standard QM accommodates this mathematically but provides no physical account of why. The iota model grounds it directly: properties are relational, so the measurement context is constitutive of the outcome, not incidental to it.

The experimental specification that follows from this: context-dependence of measured particle properties should scale predictably with the relational complexity of the measurement setup. Larger, more massive configurations have more relational modes available for the measurement context to engage. The prediction is therefore directional: context-dependence effects should be larger for more massive particles and smaller for simpler ones. This is a falsifiable claim. If systematic measurement of context-dependence shows no such scaling --- if the anomalies are random rather than structured --- the relational account is not supported.

The framework does not predict new particles. It predicts that existing anomalies are not noise. They are structure.
...
** Mathematical Status of the Framework

The framework makes three categories of claim. Each has a different mathematical status. They are not equal. Conflating them weakens the document. Separating them answers the objection that the framework lacks mathematical foundation.

*Category One: Claims that use existing mathematics.*

The view layer is described by general relativity and quantum mechanics. Those are complete mathematical frameworks — differential geometry, Riemannian manifolds, Einstein field equations, Hilbert space operators, the Schrödinger equation. The Medium does not replace this mathematics. It locates it. The claim that GR describes the view layer and QM describes substrate granularity leaking through is a placement claim, not a derivation claim. The mathematics already exists. Its domain of validity is what is being specified.

The wave function as a representation of probability density is standard probability theory. The Born rule --- probability proportional to amplitude squared --- is a mathematical relationship between the wave function and measurement outcomes. The Medium's claim that the wave function represents the relational web's probability landscape uses this mathematics without modifying it.

Entanglement as co-manifestation uses standard quantum information mathematics. The correlations are real. Their substrate ground is the claim.

*Category Two: Claims that map to known mathematical structures.*

The relational web as probability landscape maps to stochastic processes and statistical mechanics. Probability density functions, Markov processes, and the law of large numbers are the appropriate mathematical tools. The claim that stochastic substrate activity produces a smooth probability landscape is the central limit theorem operating at scale. This is known mathematics applied in a new domain. The derivation connecting substrate stochastic character to relational web probability density is the open work --- the mathematical structure is not in question.

The \iota packing geometry maps to energy minimization under packing constraints. Sphere packing mathematics, Voronoi tessellations, and the Weaire-Phelan geometry are established. The claim that \iota events self-organize into energy-minimizing configurations is a claim about which of these structures applies and why. The mathematics of packing exists. The application to \iota requires derivation.

The gravity strain field maps to continuum mechanics and elasticity theory. Strain fields obey known equations --- diffusion equations govern how disturbance propagates through a medium; elasticity equations govern how a medium responds to persistent deformation. The claim that gravity is a strain field through the \iota medium is a claim that these mathematical structures describe it. Showing that the strain field equations recover the inverse square law in the appropriate limit is the specific derivation target.

The adjacency constraint maps to graph theory and combinatorics. Constraint satisfaction over relational graphs is established mathematics. The claim that identical adjacent distinction characters are forbidden is a constraint on valid configurations in a relational graph. The combinatorial consequences --- which configurations are possible, what the minimum threshold is --- follow from this constraint by graph-theoretic analysis.

*Category Three: Claims requiring new derivation.*

The combination rules for \iota relational modes have no existing mathematical framework to map to. The modes are qualitatively distinct. The rules governing how they combine --- what produces stability, what produces the observed particle structure --- require derivation from the mode structure itself. No existing mathematics is being applied. This is genuinely new territory.

The formal account of how an embedded observer constructs a dimensional coordinate system from the pre-dimensional relational web does not exist. This is the primary open derivation of the framework. Until it exists, the correspondence between substrate structure and dimensional physics rests on conceptual argument rather than mathematical derivation.

The path from \iota mode structure to the Hamiltonian requires the combination rules first. It cannot proceed until Category Three's first item is resolved.

*The honest position.*

The view layer mathematics is complete and correct. The relational web mathematics is mappable to known structures --- the derivations connecting them to \iota are open work, not open questions about which mathematics applies. The substrate-to-mode-structure derivation is genuinely new. The framework is not mathematically empty. It is mathematically stratified --- complete at one level, mappable at the second, frontier at the third.

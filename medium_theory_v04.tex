...
From this primitive a three-level hierarchy derived by successive combination. At the first level, \(\kappa\) = \(\sum\) \(\iota\) is the coupling character of a local collection. At the second level, K = \(\sum\) \(\kappa\) is the total configuration energy. At the third level, K projects into spacetime as \(\hat{H}\) , the Hamiltonian of standard physics — not assumed but derived.
...
\subsection{Axioms}
\label{sec:orgc6a7f1d}

\subsubsection{Axiom I --- The Eternal Medium}
\label{sec:orgb90d4e4}
...
\label{sec:orge78b4e8}

\subsubsection{Primitive I --- The Field}
\label{sec:org6cfeb60}
...
The four claims are:
\begin{enumerate}
\item Entropy is not a property of the universe. It is local to an observer's coarse-graining of accessible microstates, and exists only at the projection layer.
\item The arrow of time is not a substrate property. Temporal directionality is itself a projection-layer artifact, which means frequency — and therefore redshift — is doubly derived.
\item Cosmological redshift is a multi-layer projection artifact. It does not require expanding space, recession velocity, or a singular origin event.
\item The Big Bang is not a falsified theory. It is a malformed question — a projection error that dissolves when the substrate/projection distinction is applied consistently.
\end{enumerate}
...
\begin{enumerate}
\item Entropy as Emergent Accounting
\end{enumerate}
...
\begin{enumerate}
\item Time as Projection and Its Consequences for Frequency
\end{enumerate}
...
\begin{enumerate}
\item Redshift as Multi-Layer Projection Artifact
\end{enumerate}
...
\begin{enumerate}
\item The Big Bang as Projection Error
\end{enumerate}
...
\url{https://orcid.org/0009-0001-0328-2299}
\end{document}

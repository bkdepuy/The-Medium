...
What it asks instead is the question: \emph{from where does all of this begin?} The equations of physics are precise maps. The challenge is to identify the territory they map --- not the territory of observable phenomena, which physics has surveyed magnificently, but the substrate that gives rise to a universe in which observation is possible at all.

There is a limitation that physics has not confronted fully. Anyone who has ever done physics is an earth-bound observer. That observer arrived late --- billions of years after the universe began --- in the midst of an apparent unlimited universe. That observer equipped with sensory tools required for it's survival. Once assured of it's survival it began to ask why and how do things work.  Using a dimensional coordinate system (meters, seconds, joules) built to understand their surroundings. The language of physics constructed from the observation point it attempts to describe. Its constants, dimensions, and equations carry the fingerprints of that position.
...
\label{sec:orgc5cdcbf}

The substrate description is co melmplete in itself. It contains no dimensions, no observers, no physics. It is what it is.
\subsection{The Foundational Claim}
\label{sec:org5cf13fc}
...
\subsection{Axioms}
\label{sec:orgdb20185}

\subsubsection{Axiom I --- The Eternal Medium}
\label{sec:org3be2088}
